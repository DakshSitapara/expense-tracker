...
<main className="max-w-7xl mx-auto px-4 py-2 items-center">
  ...
  {expenses.length === 0 ? (
    <div className="text-center py-10 text-gray-500 dark:text-gray-400">
      <p className="text-md">Start by adding your first expense to see your dashboard here.</p>
    </div>
  ) : (
    <>
      <p className="text-center text-lg font-medium text-gray-600 dark:text-gray-300">
        Total Amount:{" "}
        <span className="text-primary dark:text-primary-light">₹{totalExpenses}</span>{" "}
        | Total Expenses:{" "}
        <span className="text-primary dark:text-primary-light">{totalExpensesCount}</span>
      </p>
      ...
      <div
        id="expense-table"
        className="max-w-2xl mx-auto bg-transparent dark:bg-gray-900 rounded-xl shadow border border-gray-200 dark:border-gray-700 overflow-x-auto"
      >
            ...
            {loading ? (
              ...
              paginatedExpenses.map((expense, idx) => (
                <TableRow key={expense.id}>
                  ...
                  <TableCell className="px-2 py-1 font-semibold text-primary dark:text-primary-light">
                    ₹{expense.amount}
                  </TableCell>
                  <TableCell className="px-2 py-1">
                  ...
                  <TableCell className="px-2 py-1 text-gray-700 dark:text-gray-300">
                    {expense.title}
                  </TableCell>
                  <TableCell className="px-2 py-1 text-gray-600 dark:text-gray-400">
                  ...
                  <TableCell className="px-2 py-1">
                    <div className="flex items-center gap-2">
                      <Popover
                        ...
                        onOpenChange={(open) =>
                          setPopoverOpen({ ...popoverOpen, [expense.id]: open })
                        }
                      >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              navigator.clipboard.writeText(
                                `You spent ₹${expense.amount} on "${expense.title}" on ${new Date(
                                  expense.date
                                ).toLocaleDateString()}.`
                              );
                              toast.success(
